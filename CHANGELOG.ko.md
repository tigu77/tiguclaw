...
- **Codex 서브에이전트가 웹 검색을 완료했는데도 도구를 사용하지 않았다고 경고하던 문제를 수정했습니다.**
- **비서가 사용자가 쓰는 언어로 답합니다.** 비서가 직접 쓰는 작업 결과 보고도 그 대화의 언어를 따릅니다(실패·중단 같은 고정 알림 문구는 아직 한국어입니다).
- **텔레그램에서 시킨 백그라운드 작업의 결과 보고·알림이 텔레그램으로 오지 않던 문제를 수정했습니다.** 대시보드 세션에서 이어지는 대화였을 때 생겼고(특히 서브에이전트), 비서는 보고했다고 알고 있었습니다.
- **백그라운드 작업 결과 보고에 텔레그램 답글을 달면 작업을 시작한 세션이 아니라 기본 세션으로 가던 문제를 수정했습니다.**
- **iPhone에서 채팅 맨 끝을 넘겨 끌면 상단 헤더와 세션 탭이 밀려 올라가 가려지던 문제를 수정했습니다.**
...
- **긴 대화의 요약 지침을 보완했습니다.** 완료·취소된 작업, 변경된 지시와 계속 지켜야 할 제약을 구분해 남기도록 했습니다.

### Fixed

- **Codex 등으로 오래 작업한 뒤 Claude로 돌아오면 최근 대화가 누락되던 문제를 수정했습니다.** 최근 기록을 자르기 전에 세션 소속으로 전환 경계를 찾습니다.
- **대시보드에서 긴 작업의 응답 대기가 약 5분 뒤 실패하던 문제를 수정했습니다.** 메시지 응답 대기에 별도 시간제한을 두지 않고, 브라우저 연결 종료 시 대기 연결을 정리합니다.
- **진행 중 HTTP 요청 때문에 브리지 종료가 완료되지 않던 문제를 수정했습니다.** 종료 시 새 연결을 막은 뒤 기존 연결을 정리합니다.
- **Windows에서 셸 명령의 인자가 손상되거나 백그라운드 셸·훅 실행 시 명령 창이 나타나던 문제를 수정했습니다.**
- **완료된 백그라운드 작업이 중단된 것으로 표시되거나, 접힌 그룹의 답변이 클릭해도 펼쳐지지 않던 문제를 수정했습니다.**
- **사용량 표시와 집계를 보완했습니다.** 채팅·작업 카드·게이트웨이의 턴 집계 기준을 맞추고, 실패 시도와 Codex 내부 재시도에서 사용량을 확인하지 못한 경우 합계가 완전한 것처럼 표시하지 않습니다.
...
- **외부 MCP·플러그인 도구가 실패 뒤 두 번 실행될 수 있던 것**을 고쳤습니다. 이름이 읽기
  전용처럼 보여도 다시 실행하지 않습니다. 재실행을 막은 경우에도 한도 안내와 오류 보고는
  그대로 전달됩니다.
- **OpenAI 계열 모델에서 긴 대화의 앞부분이 요약 없이 사라지던 것**을 고쳤습니다. 이제 다른
  모델과 같은 방식으로 요약해 남기고, 압축이 일어나면 화면에 알립니다.
- **일부 codex 모델에서 대화 요약이 계속 실패하던 것**을 고쳤습니다. 실패하는 동안 옛 대화가
  요약 없이 잘려 나갔습니다.
- **압축이 실패해도 «압축 중» 표시가 사라지지 않던 것**을 고쳤습니다. 이제 실패·건너뜀도
  화면에 알립니다. Claude 모델에서는 압축이 성공해도 표시가 남아 있었습니다.
- **대시보드에서 10MB 넘는 파일이 첨부되지 않던 것**을 고쳤습니다. 상한은 20MB 로 올랐는데
  화면에는 옛 상한이 그대로 남아 있어, 보내기도 전에 거절했습니다. 이제 상한을 서버에서
  받아 씁니다.
...
- **첨부 파일 상한을 올렸습니다.** 파일 하나당 20MB, 한 번에 보낼 수 있는 합계 50MB 입니다.
  ...
  (★이 판에서는 **서버 상한만** 올랐습니다 — 대시보드 화면은 옛 상한을 그대로 들고 있어,
  실제로 양쪽에서 되는 것은 0.54.1 부터입니다.)
...
- **Claude 모델도 «빠름» 으로 돌릴 수 있습니다.** 0.52.0 은 codex 에만 적용돼서, 같은
  설정을 적어도 모델에 따라 다르게 굴었습니다. 안 켜졌을 때는 **왜 안 켜졌는지**가
  로그에 남습니다(계정에서 추가 사용량을 켜야 하는 경우 등).

### Fixed

- **«빠름» 의 대가를 화면이 잘못 말하던 것**을 고쳤습니다. 직접 정의한 provider 이름으로
  Claude 를 쓰면 «이 provider 는 안 읽음» 이라고 나오면서 **실제로는 2배 단가가 청구**
  됐습니다. 이제 codex 는 «크레딧 2.5배», Claude 는 «비용 2배» 로 각각 맞게 나옵니다.
- 변경 내역(v0.52.0)에서 **겪은 적 없는 항목들을 걷어냈습니다.** 그 판 안에서 생겼다
  없어진 결함들이 마치 쓰던 중에 겪은 것처럼 적혀 있었습니다.

## [0.52.0] - 2026-09-10

### Added

- **모델 프로파일에 «빠름» 을 켤 수 있습니다**(현재 codex 계열만). 응답이 1.5~2배
  빨라지는 대신 **크레딧을 2.5배** 씁니다. 기본은 꺼짐이고, `/models` 가 어디에 켜져
  있는지와 그 대가를 함께 보여줍니다 — 이 값을 읽지 않는 provider 에는 그렇게 적습니다.
- `/models` 가 **실제로 실려 나가는 추론 강도**를 항상 보여줍니다 — 그 값이 프로파일·설정·
  모델 기본 중 어디서 왔는지까지. 종전엔 프로파일에 직접 적었을 때만 보였습니다.
- **잡 아이콘을 바꿀 수 있습니다.** 매니저·서브에이전트 아이콘이 코드에서 빠져
  `<홈>/locales/<언어>.json` 으로 덮입니다.
- 플러그인이 기본 스킬·에이전트·커맨드·엔드포인트와 **같은 이름을 내면 경고**가 뜹니다.
  막지는 않지만, 능력이 조용히 바꿔치기되지 않습니다.

### Changed

- **채팅 카드 접기를 다시 만들었습니다.** 답변 메시지도 접히고(종전엔 도구 스텝 카드만),
  머리줄뿐 아니라 **본문 아무 데나** 눌러도 접히며, 접어도 **세 줄은 남습니다.** 접기
  삼각형도 1.7배 커졌습니다.
- 접기의 기본은 **펼침**입니다 — 직전 답변이 저절로 접히던 것을 없앴습니다. 글을 드래그로
  선택해도 접히지 않습니다.
- 매니저가 🎖️ **금색 배지**로 서브에이전트(보라)와 구분됩니다.
- 백그라운드 작업 카드가 **오래된 순**으로 쌓입니다.
- **위임받아 도는 서브에이전트가 대화를 이름 바꾸거나 보관하지 못합니다.** 1턴 돌고 끝나는
  위임이 사용자 대화를 건드릴 수 있었습니다. 대화 검색·조회는 그대로입니다.

### Fixed

- **연결이 끊긴 외부 MCP 서버 하나가 대화 한 턴을 통째로 죽이던 것**을 고쳤습니다.
  대상 앱을 껐다 켜면 다음 턴에 돌아옵니다.
- **OpenAI 어댑터에서 플러그인 도구 이름이 기본 도구와 겹치면 그 턴이 통째로 죽던 것**을
  고쳤습니다. 이제 세 어댑터가 같은 판정을 씁니다.
- 채팅을 위로 스크롤해 이력을 불러오는 순간 **화면이 비어버리던 것**을 고쳤습니다. 가만히
  두면 빈 채로 멈춰 있었습니다.
- 채널 목록이 조용히 갱신되지 않던 것을 고쳤습니다.
- **스킬이 조용히 사라지던 것**을 고쳤습니다. 이제 못 싣는 이유와 고치는 법이 로그에 남습니다.
- 연결이 끊겼을 때 로그에 `terminated` 만 남아 **왜 끊겼는지 알 수 없던 것**을 고쳤습니다.
- 캐시 경고가 근거 없이 «모델을 바꿔 보라» 고 처방하던 것을 뺐습니다. 대신 로그가 **캐시가
  어디서 깨졌는지**를 직접 짚습니다.
...
- 위임받은 에이전트·매니저가 **자기가 할 수 있는 것만** 안내받습니다. 종전엔 작동 헌법과
  스킬 목록을 비서와 똑같이 받았는데, 그중엔 그 자리에서 쓸 수 없는 도구를 쓰라는 지시가
  섞여 있었습니다. 능력은 그대로입니다 — 필요하면 `find_skills` 로 찾고 이름을 지정하면
  그대로 씁니다.
- `find_skills` 를 **인자 없이** 부르면 쓸 수 있는 스킬 전체 목록을 돌려줍니다. 종전엔
  검색어가 필수라, 무엇이 있는지 모르면 찾을 방법이 없었습니다.
- 스킬 목록이 **바이트 상한**을 지킵니다. 종전엔 개수만 제한해서, 설명이 긴 스킬이 몇 개만
  있어도 목록이 무한정 커질 수 있었습니다.

### Removed

- `schedule-safety-check` 스킬을 뺐습니다. 스킬이 하던 일(예약을 걸기 전에 그 프롬프트가
  나중에 무인으로 실행돼도 안전한지 따지는 것)은 **비서의 기본 동작으로 옮겼습니다** —
  이제 위임받은 턴에서도 같은 검사가 걸립니다.
