...
- **도구를 실행한 뒤 실패했을 때 그 도구가 두 번 실행될 수 있던 것.** 외부 MCP·플러그인
  도구는 이름이 읽기 전용처럼 보여도 다시 실행하지 않습니다. 재실행을 막은 경우에도
  한도 안내와 오류 보고는 정상적으로 전달됩니다.
- **OpenAI 계열 모델에서 긴 대화의 앞부분이 요약 없이 사라지던 것.** 이제 다른 모델과
  같은 방식으로 요약해 남기고, 압축이 일어나면 화면에 알립니다.
- **codex 대화 요약이 일부 모델에서 계속 실패하던 것.** 요약 호출이 모델 프로파일의 추론
  강도를 쓰지 않고 자체 값을 보내, 그 값을 받지 않는 모델에서는 매번 실패하고 오래된 대화가
  요약 없이 잘렸습니다.
- **압축이 실패해도 «압축 중» 표시가 사라지지 않던 것.** 이제 실패·건너뜀도 화면에 알립니다.
  Claude 모델에서는 압축이 성공해도 표시가 남아 있었습니다.
- **대시보드에서 10MB 넘는 파일이 첨부되지 않던 것.** 상한을 20MB 로 올렸지만 화면이 옛
  상한을 들고 있어, 보내기도 전에 거절했습니다. 이제 상한을 서버에서 받아 씁니다.
