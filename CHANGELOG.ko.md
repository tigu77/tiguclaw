...
- **대시보드 접기를 다시 만들었습니다.** 답변 메시지도 접히고(종전엔 도구 스텝만), 머리줄뿐
  아니라 **본문 아무 데나** 눌러도 접히며, 접어도 **세 줄은 남습니다.** 접기 삼각형도 1.7배
  커졌습니다.
...
- **백그라운드 작업 카드가 어느 순간부터 화면 밖에서 진행되던 것**을 고쳤습니다. 드로어를
  열어둔 채 작업이 쌓이면 세 번째쯤부터 새 카드가 안 따라왔고, 한 번 끊기면 다시 붙지
  않았습니다. 새로고침했을 때 가장 오래된 작업이 아니라 **최신**이 보이도록 함께 고쳤습니다.
- **접기 삼각형이 접힌 상태를 표시하지 않던 것**을 고쳤습니다. 접었는데도 펼침 모양(▼)
  그대로였습니다.
...
- 채팅을 위로 스크롤해 이력을 불러오는 순간 **화면이 비어버리던 것**을 고쳤습니다. 가만히
  두면 빈 채로 멈춰 있었습니다.
- 위로 스크롤하면 **같은 메시지가 두 번 나오던 것**을 고쳤습니다.
- 새로고침하면 **백그라운드 카드 순서가 뒤집히던 것**을 고쳤습니다. 그 상태에서 상한을
  넘으면 방금 끝난 작업부터 지워졌습니다.
- 셸 명령줄이 매니저 금색으로 물들던 것과, 채널 목록이 조용히 갱신되지 않던 것을 고쳤습니다.
