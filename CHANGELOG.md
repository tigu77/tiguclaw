...
**배경 작업이 «완료했습니다» 라고 해놓고 한 시간 반을 더 일하던 것.** 맡긴 일을 끝내고 마감
보고까지 쓴 매니저가, 뒤늦게 도착한 검토 결과를 보고 **새 작업자를 일곱이나 더 띄우며 코드를
계속 고쳤습니다**(실측 86분). 사용자가 직접 멈춰야 했고, 그 사이 작업 트리가 다시 미완성이
됐습니다. 원인은 권한이 아니라 **맥락**이었습니다 — 결과를 받는 시점이 처음 요청에서 한참
멀어, 늦게 온 검토 의견을 «원래 요청의 완료조건»이 아니라 «해야 할 새 일»로 읽은 것입니다.

### Fixed

- **결과를 받을 때 «원래 맡은 일» 을 다시 보여줍니다.** 배경 작업이 자식의 결과를 받아
  마무리하는 시점은 처음 요청에서 한참 멀고(도구를 수십 번 쓴 뒤), 대화 기록도 압축됩니다.
  그래서 늦게 온 검토 의견을 **«원래 요청의 완료조건»이 아니라 «해야 할 새 일»로** 읽는
  일이 생겼습니다. 이제 그 시점에 원래 맡은 일을 다시 싣고, 자식의 결과는 **새 지시가 아니라
  참고 증거**라고 명시합니다 — 완료조건을 실제로 깨면 이어서 고치고, 그렇지 않은 개선
  제안이면 고치지 말고 보고에만 적습니다.
  ★**권한은 그대로입니다.** 배경 작업은 여전히 맡은 일 전체를 소유하고, 늦게 발견된 진짜
  문제는 **스스로 고칩니다.** 한때 이 시점의 도구를 막는 방식도 검토했지만, 그러면 정말
  고쳐야 할 것도 못 고치게 되어 택하지 않았습니다.
- **도구 한도에 걸린 배경 작업에게 «최종 요약» 을 시켜놓고 계속 돌리던 것.** 한도에 닿으면
  마무리를 요청하는데, 그 문구가 *"최종 요약을 쓰라"* 여서 모델은 **완료 문장**을 썼습니다.
  그런데 배경 작업은 그 뒤로 자동으로 이어집니다 — 즉 «완료» 라고 해놓고 계속 도는 것처럼
  보였습니다. 이제 이어갈 때는 «중간 점검» 을 요청합니다.
- **긴 배경 작업이 끝없이 돌 수 있던 것.** 도구 한도를 넘겨 이어갈 때 누적 상한이 없었습니다.
- **모델을 못 쓴 이유를 설명하는 문장이 잘려나가던 것.** 상류 오류 본문이 길면 정작 필요한
  해설이 통째로 잘렸습니다.
- **컨텍스트가 작은 모델의 프롬프트 잘림 판정**에 애매한 구간을 더했습니다 — 단정할 수 없을
  때는 *"잘렸을 수 있습니다"* 로 말합니다.
- **같은 실패가 기억에 계속 새로 쌓이던 것**(작업 id 가 달라 매번 별개로 기록)을 한 번 더
  좁혔습니다.
- **로그가 자식 결과의 전달 경로를 틀리게 말하던 것.** 실제로는 결과 수신함인데 로그만 옛
  이름(«steering 큐»)이었습니다. 그리고 배경 작업 로그에 **«거두기 턴» 이라는 표시가 없어**,
  이번 사고를 재구성할 때 다른 신호로 추론해야 했습니다 — 이제 한 줄로 남습니다.

### Changed

- **구독 로그인이 플러그인이 됐습니다.** Claude·Codex 구독 인증을 코어에서 **번들 플러그인**
  으로 분리했습니다(`plugins/claude-subscription-auth`, `plugins/codex-subscription-auth`).
  해당 폴더를 빼면 그 구독 경로가 조용히 꺼지고(API 키 사용자는 그대로), 코어에는 인증을
  등록하는 코드가 남지 않습니다. 번들 플러그인은 8개에서 10개가 됐습니다.
- **홈에 직접 깐 플러그인도 구독 인증을 제공할 수 있습니다.** 같은 이름을 **먼저 잡은 쪽이**
  가져갑니다. (Claude 쪽은 홈 플러그인으로 옮길 수 있고, Codex 쪽은 토큰 갱신 때문에 아직
  번들 전용입니다.)
...
Codex)에서만 가져와서, OpenAI 키나 OpenRouter·Groq 같은 서비스를 붙여도 **무슨 모델이
있는지 물어볼 곳이 없었습니다.** 이름을 정확히 알아야만 쓸 수 있었어요.

### Added

- **붙인 모든 곳에서 모델 목록을 가져옵니다.** OpenAI 호환 엔드포인트면 표준 경로로
  조회합니다 — 설정에 세 줄 적어 OpenRouter 를 붙이면 수백 개가 바로 잡힙니다.
  ★단, **목록에 있다고 다 쓸 수 있는 건 아닙니다** — 벤더가 오래된 모델을 목록에만 남겨두는
  경우가 있어, 고른 게 안 되면 아래 «이유» 가 알려줍니다.
- **모델을 고를 때 그 모델이 뭘 할 수 있는지 보입니다.** `/models` 의 각 모델 옆에
  `[131K · 도구✅]` 처럼 컨텍스트 크기와 도구 사용 가능 여부가 붙습니다.
  ★**모르는 건 «안 됨» 이라고 하지 않습니다** — 벤더가 안 알려주면 아무 표시도 안 붙습니다.
  화면이 모르는 걸 아는 척하지 않게 했습니다.
- **모델이 프롬프트를 잘라내면 알려줍니다.** 컨텍스트가 작은 모델(특히 로컬 모델의 기본
  설정)은 지침·기억·능력 목록을 **조용히 버리고** 답합니다 — 오류도 안 나고 답도 나오는데
  내용만 빠집니다. 이제 보내기 전·후로 *"이 모델이 우리 프롬프트를 잘라냈습니다 —
  최소 24,553토큰을 보냈는데 4,095토큰만 처리했습니다"* 처럼 수치와 함께 말합니다.

### Fixed

- **지정한 모델을 못 쓸 때 이유를 말합니다.** 종전엔 *"쓸 수 없어 기본 모델로 답했습니다"*
  뿐이라 **이름을 잘못 썼는지, 키가 없는지, 그 모델이 그 기능을 못 하는지** 구분할 수
  없었습니다(진짜 이유는 서버 로그에만 있었고, 원격에서 쓰는 분은 그것도 못 봅니다).
- **긴 배경 작업이 도구 사용 한도에서 잘리던 것.** 실제로 파일을 고쳐가며 진행하던 작업이
  한도에 닿아 중단됐고, 다시 실행하면 처음부터 반복해 **영영 끝나지 않았습니다.** 이제
  중간 정리를 하고 **이어서 계속합니다.** (사용자가 기다리는 대화는 그대로 — 이어가기는
  백그라운드 작업에만 붙습니다.)
- **같은 실패가 기억에 계속 새로 쌓이던 것.** 작업 id 만 다르면 같은 오류가 매번 별개로
  기록돼, 기억이 중복으로 차고 «반복되는 문제» 로 알아채는 것도 늦어졌습니다.
- **아무 일도 없던 주의 회고까지 매주 기록되던 것.** *"이번 주 신호 0 — 정상 운영"* 이라는
  내용 없는 항목이 매주 하나씩 쌓여, 정작 필요한 기억을 밀어냈습니다.



## [0.43.2] - 2026-08-31

플러그인 주변을 이틀 들여다봤더니 열 가지가 걸렸습니다. 전부 "원래 이랬어야 하는
것" 이라 고쳤습니다. 새로 생긴 기능은 없습니다.

### Fixed

- **플러그인이 무엇을 요구하는지가 한국어로만 보였습니다.** 영어로 쓰시는 분은 목록에서
  `외부 api.example.com · 스스로 말함` 같은 글자를 그대로 만났습니다. 하필 보안 문서가
  *"설치 전에 여기서 확인하세요"* 라고 가리키는 자리라, 보기 흠이 아니라 **판단할 근거가
  안 닿는** 문제였습니다. 이제 화면 언어대로 나옵니다.
- **플러그인을 지워도 다시 켜지면 살아났습니다.** 배선만 걷고 "껐다" 는 기록을 안 남겨서,
  업데이트나 재시작 한 번이면 돌아왔습니다. 확인 문구도 이제 그 사실을 정확히 말합니다.
- **전에 껐던 이름을 다시 깔면 꺼진 채로 떴습니다.** 설치했는데 아무 일도 안 일어나니
  설치가 실패한 줄 알기 쉬웠습니다. 설치는 곧 "켜라" 이므로 그 기록을 지웁니다.
- **위젯 설정에서 자격증명이 새지 않는지 다시 확인했습니다.** 이번 개발 중에 `chartKey`·
  `sortKey` 같은 평범한 이름이 막히는 걸 풀어보려다, `passphrase`·`signingKey`·
  `encryptionKey` 처럼 **진짜 열쇠 24종이 함께 뚫리는 것**을 출시 전 검토에서 잡았습니다.
  되돌렸고, 이제 그 24종은 검사가 붙잡고 있습니다.
  그래서 `chartKey` 류가 막히는 것은 **그대로입니다.** 이름만으로는 `apiKey` 와 `chartKey`
  를 가를 수 없어서(문법이 같습니다) 막는 쪽을 기본으로 뒀습니다 — 막히면 사유와 함께
  거부되고 `.env` 로 두라는 안내가 붙지만, 새면 값이 화면과 백업에 남아 되돌릴 수 없습니다.
  그리고 **값도 봅니다.** 이름에 걸릴 낱말이 없어도 값이 개인키(`-----BEGIN …`)·GitHub·
  Slack·OpenAI 토큰이거나 `postgres://사용자:비밀번호@…` 형태면 막습니다 — `pem`·
  `kubeconfig`·`databaseUrl` 처럼 이름만으로는 못 잡던 것들입니다.
  다만 `otp: "123456"` 처럼 값이 평범한 것은 못 잡습니다. 자격증명을 쓰는 위젯이라면
  설정 스펙에 **secret 으로 선언**하세요 — 추측은 선언을 대신하지 못합니다.
- **홈 플러그인이 앱과 함께 온 플러그인의 이름을 가로챌 수 있었습니다.** 같은 이름을
  막는 검사가 *"지금 돌고 있나"* 를 물어서, 그 플러그인을 꺼둔 순간 문이 열렸습니다.
  그러면 목록에는 **앱과 함께 온 것의 출처·버전·권한**이 뜨는데 실제로는 직접 깐 코드가
  돌았습니다 — 화면이 *"외부에 아무것도 안 합니다"* 라고 하는데 밖으로 나가겠다고 선언한
  코드가 도는 상태였습니다. 이제 **디스크에 그 이름이 있으면 예약**됩니다(그 플러그인이
  꺼져 있든 로드에 실패했든).
- **플러그인 화면에서 대시보드를 끄면 아무것도 묻지 않고 꺼졌습니다.** 화면이 그 자리에서
  사라지고, 그 상태가 기록으로 남아 다시 켜도 안 돌아왔습니다 — 되돌릴 문이 방금 없앤 그
  화면이었습니다. 모듈 화면에는 확인 절차가 있었는데 이쪽에만 없었습니다. 이제 양쪽 다
  묻습니다.
- **`npm start` 로 띄우면 플러그인이 하나도 안 붙었습니다.** 대시보드도 텔레그램도
  스케줄러도 없이 조용히 반쪽으로 떴습니다. 엉뚱한 산출물을 가리키고 있었습니다.
- **플러그인 설정을 바꾸다 실패하면 화면 언어가 아닌 문장이 떴습니다.** 목록의 요구사항은
  화면 언어대로 나오게 고쳤는데, **설정 한 칸을 바꾸다 거절당하는 자리**가 같이 안
  고쳐졌습니다 — 영어로 쓰시는 분은 *"'apiKey' 는 secret 입니다…"* 같은 한국어를,
  한국어로 쓰시는 분은 반대로 영어를 만났습니다. 이제 양쪽 다 화면 언어대로 나옵니다.
- **멀쩡한 플러그인이 로그에 매번 오류로 찍혔습니다.** 플러그인 설정을 한 번이라도
  바꾸면 그 값이 `<홈>/plugins/<이름>/` 에 저장되는데, 시작할 때 그 폴더를 플러그인으로
  읽으려다 실패해서 *"weather load: ENOENT …"* 같은 줄이 남았습니다. 해당 플러그인은
  멀쩡한데 깨진 것처럼 보였고, 로그를 볼 때 **진짜 문제를 가렸습니다.** 이제 사실대로
  한 줄만 남습니다.
- **플러그인 이름에 대문자를 쓰면 아무 일도 안 일어났습니다.** 이름에 쓸 수 있는 글자가
  문서 어디에도 없었고, 규칙을 어기면 화면엔 *"쓸 수 있는 플러그인을 못 찾았습니다"* 만
  뜨고 진짜 이유는 로그에만 남았습니다 — 자기 코드를 의심하게 되는 자리입니다.
  [만드는 법](docs/plugins.md) §2 에 규칙(소문자·숫자·하이픈)과 **안 뜨면 이름부터
  보세요** 를 적었습니다.

### Changed

- **보안 문서에 확장 이야기를 넣었습니다**([`docs/security.md`](docs/security.md)).
  플러그인은 데몬과 같은 프로세스에서 돌고 격리가 없습니다 — `needs` 는 **선언이지 감옥이
  아닙니다.** 그리고 **앱과 함께 온 것**(대시보드·텔레그램)과 **직접 설치한 것**은 판단이
  다르므로 구분해 적었습니다.
