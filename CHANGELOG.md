...
### Fixed
- **응답 백엔드가 보고한 실패 사유가 그대로 보입니다.** 이 백엔드는 요청이 정상 접수된 뒤(HTTP 200) 스트림 안에서 실패를 통보하는데, 종전에는 그 사유를 읽지 못해 "모델이 빈 응답을 냈다"로 잘못 진단하고 같은 요청을 세 번 더 시도한 뒤 엉뚱한 오류를 표시했습니다. 이제 백엔드가 준 코드와 메시지를 그대로 로그와 오류에 싣습니다. 작업 중 실제로 변경을 수행한 뒤 실패한 경우에는 다른 모델로 재실행하지 않습니다(같은 변경이 두 번 적용되는 것 방지).
