...
### Fixed
- **응답 백엔드가 실패했을 때 작업이 중간에 멈추지 않고 자동으로 다시 시도합니다.** 이 백엔드는 요청이 정상 접수된 뒤(HTTP 200) 스트림 안에서 실패를 통보하는데, 종전에는 그것을 "모델이 빈 응답을 냈다"로 잘못 진단해 같은 요청을 세 번 더 조른 뒤 작업 도중에 턴을 끝냈습니다. 이제 서버 오류와 같은 부류로 보고 같은 요청을 짧은 간격으로 두 번까지 재전송하며, 그래도 실패하면 백엔드가 준 실제 사유를 로그와 오류에 표시합니다. 이미 변경을 수행한 뒤 실패한 경우에는 다른 모델로 재실행하지 않습니다(같은 변경이 두 번 적용되는 것 방지).
