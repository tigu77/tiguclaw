...
- **현재 세션에 진행 중인 백그라운드 작업이 입력창 위에 표시됩니다.** 지금 보고 있는 세션에서 돌고 있는 백그라운드 작업(워커) 수가 채팅 입력창 위에 "🔄 이 세션 백그라운드 작업 N개 진행 중"으로 뜨고, 누르면 작업 드로어가 열립니다(세션별로 집계, 탭을 바꾸면 그 세션 기준으로 갱신).
