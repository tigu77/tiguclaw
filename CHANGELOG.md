...
- **대화가 왜 그 시점에 끝났는지 로그로 확인됩니다.** "하겠다고 해놓고 멈춘 것 같다"는 상황을 진단할 때, 종전에는 판단 근거가 로그에 없어 추측에 의존해야 했습니다. 턴이 끝날 때 판정 재료(중간에 끼어든 메시지 수, 마지막 응답 길이, 이후 실행한 도구)를 한 줄로 남깁니다. 함께, 정상 동작인 mid-turn 메시지 주입이 오류 수준으로 기록되어 진짜 오류를 가리던 것도 고쳤습니다.
