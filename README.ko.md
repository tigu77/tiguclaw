...
웹 대시보드는 데몬이 알아서 같이 띄웁니다 — 따로 실행할 명령이 없습니다. 데몬이 떠 있으면:
...
> 곧 권한입니다.** 폰에서 쓰고 싶으면 포트를 열지 말고 사설 네트워크로 터널링하세요(예:
...
`/models` 는 **지금 실제로 실려 나가는 강도**와 그 값이 어디서 왔는지(프로파일·설정·모델
기본)까지 보여줍니다. 전역을 바꿨는데 안 먹는 것 같을 때 여기서 확인하세요.

### 응답 빠르게 하기

응답이 더 빨리 오게 할 수 있습니다. **대가가 있는 선택**이라 기본은 꺼져 있고, 모델 프로파일의
모델마다 따로 켭니다:
...
codex 계열은 **크레딧을 2.5배**, Claude 는 **비용이 2배**입니다. OpenAI 모델도 켤 수 있지만
배수는 아직 재지 않았습니다. `/models` 가 어디에 켜져 있는지와 그 대가를 함께 보여주고,
켜지지 않으면 로그가 이유를 말합니다(계정 설정 등).
...
여덟 가지입니다. 나머지는 전부 [전체 기능 목록](docs/features.ko.md)에 있습니다.
...
6. **말로 확장** — 새 슬래시 명령·HTTP 엔드포인트·예약 작업·재사용 스킬을 코어 패치 없이
   홈 아래 *데이터*로 추가합니다.
7. **화면을 보고 손을 씁니다** — 비서가 그 컴퓨터의 화면을 보고 클릭·글자 입력·단축키·
   드래그·스크롤로 조작합니다. macOS·Windows 에서 따로 설치할 것 없이 돕니다.
   ★**되돌릴 수 없는 일입니다** — macOS 는 권한을 주기 전까지 아무것도 못 하지만
   Windows 는 따로 묻지 않습니다. 쓰지 않으려면 설정에서 끄세요.
8. **데이터는 내 컴퓨터에** — 세션·메모리·DB 전부 로컬(`~/.tiguclaw`)에.
...
| [전체 기능 목록](docs/features.ko.md) | 위 여덟 가지에 안 담긴 전부 |
