...
3. **목표를 통째로 위임** — 큰 일은 **매니저**에게 넘깁니다. 매니저가 스스로 서브에이전트를
   붙이고, **결과를 거두기 전엔 끝나지 않습니다.** 그동안 대화는 계속됩니다.
4. **여러 LLM 을 하나의 능력 표면으로** — `anthropic`·`openai`·`codex`(ChatGPT)·`ollama`(로컬)·
   `google`, 그리고 OpenAI 호환이면 무엇이든. 프로바이더 간 자동 폴백에, 어느 모델에서나 같은 도구.
5. **말로 확장** — 새 슬래시 명령·HTTP 엔드포인트·예약 작업·재사용 스킬을 코어 패치 없이
   홈 아래 *데이터*로 추가합니다.
6. **데이터는 내 컴퓨터에** — 세션·메모리·DB 전부 로컬(`~/.tiguclaw`)에.
