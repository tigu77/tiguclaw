...
- **부탁하면 스스로 업데이트** — "업데이트해줘" 한마디(또는 `/update`)면 최신 코드를 받아 재시작하고, 다 되면 알려줍니다. 손으로 `git pull` 할 필요 없어요. 기억·세션은 그대로 이어지고, 새 코드가 실행 가능하게 빌드되지 않으면 이전 버전으로 롤백해 계속 돌아갑니다.
...
- **다채널 단일 인격** — 텔레그램·CLI·HTTP, 그리고 내장 웹 대시보드 어디로 들어와도 같은 비서고 대화 맥락도 공유합니다. 대시보드는 제대로 된 채팅이에요 — 일하는 과정이 한 스텝씩 실시간으로 뜨고(각 단계가 무엇을 건드렸는지까지), 답은 흐르듯 타이핑되고, 재시작해도 대화 기록이 남습니다. 옆 패널에선 백그라운드 작업의 상태·단계·결과를 지켜볼 수 있어서, 긴 일을 대화에서 벗어나지 않고 확인합니다.
- **무거운 일도 사소한 일도 위임** — 긴 작업은 백그라운드 **매니저**에 맡겨 대화를 끊지 않고, 가벼운 일은 더 싼 모델 등급으로 내려보냅니다. 어느 일을 어느 등급에 맡길지는 모델 프로파일로 정하세요.
...
- **로그가 아니라 진짜 웹 대시보드.** 브라우저로 열면 비서가 생각하고 일하는 걸 실시간으로 봅니다 — 추론과 도구 실행이 일어난 순서대로 인터리브되고(Claude Code 웹앱처럼), 도구 카드엔 diff·출력이 리치하게 뜨고(편집은 실제 파일 줄 번호까지), 재시작해도 스크롤백이 남습니다. 입력창은 셸처럼 ↑/↓ 로 이전 입력을 꺼냅니다(세션 탭별). 여러 대화는 세션 탭으로 나란히 돌아갑니다(순서 바꾸기·탭별 입력 초안 보존). 옆 패널은 백그라운드 작업을 단계별 타임라인으로 추적하고, 선택지는 탭으로 답하고, 특정 메시지에 답글을 달거나 `#태그`로 맥락을 잡습니다.
- **주머니에 들어갑니다.** 대시보드는 데스크톱을 우겨넣은 게 아니라 진짜 모바일 UI 예요 — 서랍 내비게이션, 마스터-디테일 패널, 폰 키보드에서 제대로 동작하는 입력창. 소파에서 오래 걸리는 작업을 확인하거나 일을 시켜두고, 책상에서 같은 대화로 이어서 끝냅니다.
- **프로젝트.** `PROJECT.md` 가 있는 폴더를 가리키면 그 프로젝트 전용 스킬·서브에이전트·MCP 도구를 집어 듭니다 — 프로젝트마다 딱 맞는 능력으로 위임합니다.
- **외부 MCP 서버를 즉석에서 연결.** MCP 서버를 추가해달라고 하면 그 외부 도구를 배선합니다 — 전역으로든 특정 프로젝트로든, 코어는 안 건드리고요. Claude Code MCP 파리티, 그 이상입니다.
...
- **일하는 걸 지켜봅니다.** 서브에이전트와 오래 걸리는 매니저가 추적 가능한 잡으로 돌아서, 대시보드에서 상태·단계·결과를 따라갑니다. Claude Code 의 Task 도구를, 관측 가능하게 만든 셈입니다.
...
- **비서가 아니라 내 앱으로 답합니다.** `system` 메시지가 그대로 쓰이고 — tiguclaw 인격도, 도구도, 스킬도, 기억도 없습니다 — 게이트웨이 호출은 **내 대화에 섞이지 않습니다.** 다만 대시보드 **외부 호출 기록**에는 호출 사실이 남아요 — 주고받은 **내용은 저장하지 않고**, 어떤 모델로 몇 건을 처리했고 토큰·소요 시간·성공 여부가 어땠는지만 기록합니다(내 컴퓨터 안에). 격리된 자리에서 실행되니 `system` 을 안 보내도 비서 컨텍스트가 새지 않습니다. (다만 백엔드 로그인 계정 정보처럼 LLM 제공사가 직접 싣는 항목은 tiguclaw 가 제어하지 못합니다.)
