...
- **See what a turn costs.** Every turn shows tokens in / out and the cache-hit rate right in the chat, so waste is visible instead of theoretical. `/status` names any model that's cooling down after a rate limit — a silent fallback can't hide a provider that's been dead for days.
