...
항상 켜져 있는 내 AI 비서. Claude Code 가 하는 일은 다 하고, 거기에 여러 LLM 을 동시에 쓴다. 텔레그램·웹 대시보드·CLI·HTTP 어디로 들어가도 같은 비서다. 내 컴퓨터에서, 내 키와 내 봇으로 돌린다.
...
- **무거운 일도 사소한 일도 위임** — 긴 작업은 백그라운드 워커에 맡겨 대화를 끊지 않고, 가벼운 일은 더 싼 모델 등급으로 내려보낸다. 어느 일을 어느 등급에 맡길지는 모델 프로파일로 정한다.
...
- **멈추면 멈췄다고 말한다** — 도구가 응답 없이 붙잡혀 있으면 화면에 알리고, 텔레그램으로 대화 중이었으면 그쪽으로도 알린다. 끝내 응답이 없으면 턴을 끊는다. "돌고 있는 건지 멈춘 건지 모르겠는" 시간을 없앤다.
- **대화 이름도 말로 바꾼다** — "이 대화 이름 '핫딜 리팩터링'으로 바꿔줘" 처럼 부탁하면 된다. 여러 대화를 굴릴 때 어느 게 뭔지 헷갈리지 않는다.
...
- **로그가 아니라 진짜 웹 대시보드.** 브라우저로 열면 비서가 생각하고 일하는 걸 실시간으로 본다 — 추론과 도구 실행이 일어난 순서대로 인터리브되고(Claude Code 웹앱처럼), 도구 카드엔 diff·출력이 리치하게 뜨고(편집은 실제 파일 줄 번호까지), 재시작해도 스크롤백이 남는다. 입력창은 셸처럼 ↑/↓ 로 이전 입력을 꺼낸다(세션 탭별). 여러 대화는 세션 탭으로 나란히 돌아간다(순서 바꾸기·탭별 입력 초안 보존). 옆 패널은 백그라운드 작업을 단계별 타임라인으로 추적하고, 선택지는 탭으로 답하고, 특정 메시지에 답글을 달거나 `#태그`로 맥락을 잡는다.
...
| `SubagentStop` | 서브에이전트 종료 후 | 위임 결과 검수·기록 |
...
`<home>/settings.json` 에 블록을 추가한다. `matcher` 는 선택이고, 도구 이름을 정규식으로 받는다(비우면 모든 도구):
...
훅은 **프로젝트별로도** 걸 수 있다. 전역 `<home>/settings.json` 말고 `<프로젝트>/.tiguclaw/settings.json` 에 같은 형식으로 쓰면 그 프로젝트에서 일할 때만 돈다. Claude Code 를 쓰던 프로젝트라면 `<프로젝트>/.claude/settings.json` 의 훅도 그대로 읽는다 — 옮겨 적을 필요가 없다.

★두 층은 **덮어쓰지 않고 함께 돈다.** 전역에 안전 훅을 걸어뒀다면 프로젝트 설정이 그걸 조용히 끄는 일은 없다.

훅은 도구 호출을 **관찰하고 차단**한다. 그리고 `UserPromptSubmit`·`PreToolUse` 훅이 stdout 으로 뭔가를 쓰면 그게 **맥락으로 들어간다** — 비서가 그 내용을 보고 판단한다. 아직 안 되는 건 하나다: **도구 입력 자체를 고쳐 넣는 것**. 그건 다음 단계다.
...
> v0.21.1 까지는 MIT 였다. 그때 받아 간 버전은 계속 MIT 조건으로 쓸 수 있고, Apache-2.0 은 그 이후 버전부터 적용된다.
