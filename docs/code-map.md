...
> 건드리면 무엇이 조용히 깨지는가".
>
> ★**소스 주석이 `docs/decisions/…` 를 가리켜도 그 파일은 여기 없다** — 없는 게 정상이다
> (실측 2026-09-16: `src/`·`plugins/` 의 **34개 파일 · 48건**). 설계 이력은 개발 저장소에만
> 두고 배포본엔 안 싣는다(내부 실측·상용 전략이 섞여 있다). 주석은 **개발 저장소에서 읽는
> 글**이고, 이 배치는 의도다. 그래서 주석 48건을 고치는 대신 여기 한 번 적는다 — 손으로
> 관리하는 목록은 새 주석마다 또 어긋난다([[feedback_hand_maintained_lists]]).
> ★**비서가 읽고 실행하는 자산**(`skills/`·`agents/`)은 다르다. 거긴 자기완결이어야 하고
> 회귀 `shipped-asset-self-contained` 가 지킨다(실측 0건). 경계는 «사람이 읽는 주석» 대
> «기계가 따라가는 참조» 다.
>
> ★이 문단이 왜 생겼나: 다른 인스턴스가 설치본에서 `plugins/computer-use/src/index.ts` 의
> *"설계 정본: `docs/decisions/2026-09-15-computer-use.md`"*(배포본에 없다) 를 읽고 찾다가 못
> 찾아, **«어디서도 찾지 못했다»** 를 미해결로 보고했다. 파일은 개발 저장소에 멀쩡히 있었다.
>
