...
1. **모듈 최상단 import 부작용** — `flushEnvLoadLog`(`.env` 로드) → `net-config.js`
   import(IPv4 우선 설정). 다른 무엇보다 먼저 있어야 한다는 주석 경고 있음.
2. `initFileLogging()` → 이후 모든 console.*가 `<home>/logs/daemon-*.log`로 미러.
3. `ensureHome()` → 런타임 홈 준비/시드, `migrateLegacyAgent()`로 레포
   `AGENT.md` 1회 마이그레이션.
4. `readSystem()` 바이트 체크 — SYSTEM.md(작동 헌법) 비어있으면 부팅 로그에 경고.
5. `initStore()` → DB 준비. 직후 `setChannelSessionBindingLookup` 등록,
   `restoreCooldowns()`(모델 쿨다운 Map 복원).
6. codex OAuth self-register 모듈 optional dynamic import(실패해도 무시).
7. `initEventBus()` → `startEventPersistence(bus)`(이벤트 DB 영속)
   → `setHookObserver()`(훅 발화를 EventBus로 배선).
8. **플러그인 로딩** — `loadPlugins(appRoot()/plugins, bus)`(`core/plugins/loader.ts`).
   반환된 각 인스턴스를 capability(`channel`/`observer`/`trigger`/`service`)별로 분기해
   `getMcpServer`→MCP 등록, `channel`→`channels[]`에 push(시작은 아직 안 함),
   `trigger`/`observer`/`service`→즉시 `start*(bus)` 호출. 코어에 채널이
   하드코딩되어 있지 않다 — cli/telegram도 전부 plugin(주석).
9. `serializedHandler` 정의 + `registerWorkerHandler(serializedHandler)`(
   완료 매니저 재주입이 메인 핸들러를 다시 부를 수 있게).
10. `SIGINT`/`SIGTERM`/`unhandledRejection`/`uncaughtException` 핸들러 등록.
11. **채널 시작** — `for (const ch of channels) await ch.start(serializedHandler)`.
12. 채널 presence 등록(`setChannelPresence`).
13. `recoverInterruptedJobs()`(재시작으로 끊긴 매니저 사용자 통지) →
    `killAllBgShells` 부팅 reaper(고아 백그라운드 셸 정리).

**의존 관계 요지**: 로그 파일링 → 홈 준비 → 스토어 → 이벤트버스 → 플러그인 로드
(채널 push만, start는 아직) → 시그널 핸들러 → **채널 실제 start (리스닝 시작)** →
presence 노출 → 매니저 복구. "리스닝"은 플러그인 로드가 끝난 뒤 맨 뒤쪽 단계다.

## 2. 메시지 라우팅 경로

```
Channel.start(serializedHandler)
  → serializedHandler(msg) [index.ts]
      - /stop 등 아웃오브밴드 처리, steering 개입점 체크
      - enqueueThreadTurn(threadKey, () => handler(msg)) [thread별 직렬 큐, index.ts]
  → handler(msg) [index.ts]
      ...
      5. runUserPromptSubmitHooks() [core/entry/hook-runner.ts, index.ts]
         exit 2 → 여기서 즉시 차단 반환. exit 0 stdout → effectiveText 앞에 prepend
      6. replyToText(답글 인용) 주입
      7. route(msg, { abortSignal, session, steering }) [core/router.ts]
...
. 채널명으로 분기하는 코드는 라우터에 없음(§0 규율).

## 3. 시스템 프롬프트 조립 (`src/core/prompt-assembly.ts`)

담당 파일은 이 하나. 조립 순서·채널 배치의 **단일 정의점**은
`buildContextSlots()`이며, 3개 어댑터(claude/codex/openai, llm-runtime 소관)가
...
- **system 채널**(안 변하는 것): `system`(SYSTEM.md, `identity.ts:readSystem`),
  ...
  `selfGrowth`(SELF_GROWTH.md 확정 지침, `formatSelfGrowthDirectives`),
  `agentPathHint`, `agent`(AGENT.md, `identity.ts:readAgent`), `agentWarn`.
  AGENT.md 3인방은 일부러 **꼬리**에 둠(자주 바뀌므로 앞쪽 캐시 안 깨뜨리려고 주석).
...
- `splitSystemContext()`가 실제로 채널별로 갈라 `stable`/`volatileParts`를
  만들고, `composeSystemChannel()`이 `[어댑터 sysprompt] + [stable]`을 이어붙이며,
  `assembleUserPrompt()`가 `volatileParts`를 `<system-reminder>` 태그로 감싸
  user 프롬프트 앞에 붙인다.
...
- 각 `<pluginsRoot>/<dir>/package.json`의 `"tiguclaw"` 마커 객체를 계약으로 삼는다:
  `{ schemaVersion, kind: string|string[], name, entry }`.
- `kind`(capabilities)는 `channel|observer|trigger|service` 중 하나 이상
  `KNOWN_CAPABILITIES`). 모르는 kind만 있으면 조용히 skip.
- `entry`는 plugin 디렉토리 기준 상대경로, **default export가 인자 없는 class**여야
  한다(`new Ctor()`). 한 인스턴스가 여러 capability를 겸할 수 있다(hybrid).
- `resolveEntry()`가 built(dist)/source(tsx) 런타임을 흡수: `.ts`가 없고 `.js`
...
- 파일은 27줄, 실제 로직 없음 — 정책의 **단일 진실 소스**(데이터)만 제공:
  - `DISALLOWED_TOOLS: readonly string[] = []` — **현재 빈 배열**.
  - `DISALLOWED_URLS: readonly string[] = []` — **현재 빈 배열**.
- 실제 차단 기제(claude 어댑터, `llm-runtime/adapters/claude-agent-sdk.ts,789`):
...
- **평가**: 현재 코드베이스 시점(2026-08-08) 기준 실질 차단 리스트는 비어 있다 —
  ...
  또는 architect contract 갱신을 거친다"는 절차 주석이 있음.
...
- **hook 손자 프로세스가 stdio를 물고 있으면 PreToolUse가 영원히 pending**:
  ...
  멈추고 `/stop`도 못 풀음(hook-runner.ts, 2026-07-28 실사고).
- **재시작이 진행 중 턴을 죽여도 사용자에게 침묵**: 2026-08-01 A5 실사고
  ...
  선행(index.ts). 자식 프로세스 정리도 **1500ms force-exit 백스톱**보다
  먼저 와야 해서 shutdown 맨 앞으로 옮겨짐.
