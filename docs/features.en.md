...
- **It can see the screen and use it.** Turn it on and the assistant looks at that machine's screen, then clicks, types, presses shortcuts, drags and scrolls — on macOS and Windows, with nothing to install. ★**Those actions cannot be undone, so it ships off.** Turning it on hands over that machine's mouse and keyboard, and macOS asks separately for Screen Recording and Accessibility. It stays out of the way while you are typing or moving the mouse.
