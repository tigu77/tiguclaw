...
| `Stop` | 턴이 **정상적으로** 끝난 뒤 | 턴 후 알림·로깅 |
| `StopFailure` | 턴이 **에러로** 끝났을 때 | 실패 알림·장애 기록 |

> ★**`Stop` 과 `StopFailure` 는 상호배타입니다.** 한 턴에 둘 중 하나만 납니다 — 정상
> 종료면 `Stop`, 에러면 `StopFailure`. 「턴이 끝날 때마다」 무언가 하려면 **둘 다** 거세요.
> `StopFailure` 는 `error` 로 사용자에게 보이는 것과 **같은(비밀 가려진) 오류 문자열**을
> 받습니다.
>
> ★**사용자가 `/stop` 으로 멈춘 턴에는 둘 다 안 납니다.** 취소는 실패가 아니라서입니다.
