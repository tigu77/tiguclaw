...
위젯 자체는 `web/widget.js` 가 `tiguWidgets.register("<플러그인>/<id>")` 로 등록합니다.
**여기 적는 것은 그 목록**입니다 — 등록은 브라우저에서 일어나므로, 이 선언이 없으면 데몬은
그 플러그인에 무슨 위젯이 있는지 알 방법이 없습니다. 그러면 설정 화면에 켜고 끌 스위치도
못 그리고, 비서도 이름을 몰라 홈에 올려 달라는 말을 못 알아듣습니다.

| 키 | 뜻 |
|---|---|
| `id` | `register("<플러그인>/<id>")` 의 뒷칸과 **같아야** 합니다. 소문자·숫자·하이픈 |
| `size` | `small`(기본) 또는 `wide`. 격자 좌표가 아니라 크기 등급입니다 |
| `default` | 참이면 **설치되자마자 홈에 놓입니다**(아래 참조) |
| `labelKey` | 설정 화면의 스위치에 뜰 이름. 플러그인 자기 카탈로그의 키입니다(§7) |

> ★**`default` 는 앱과 함께 오는 플러그인에만 적용됩니다.** 사용자가 설치한 플러그인이
> 참이라고 선언해도 자동으로 놓이지 않습니다 — 홈은 사용자 자기 자리이기 때문입니다.
> 설정 화면의 스위치로는 똑같이 켤 수 있습니다.

> ★**설정이 있어야 뜻이 서는 위젯은 `default` 를 쓰지 마세요.** 예를 들어 날씨 위젯은
> 장소를 받아야 하는데, 빈 채로 홈에 놓이면 사용자는 «설정하세요» 카드만 보게 됩니다.

**한 번만 놓습니다.** 사용자가 스위치를 내리면 다시 놓지 않습니다. 순서와 크기를 바꾸는
것은 비서에게 말하면 됩니다("날씨를 맨 위로") — 스위치는 **보이기/숨기기**만 합니다.
