...
`<플러그인>:<scope>` 로 만들어집니다 — 남의 대화엔 닿지 않습니다.
...
- **위젯 설정에 열쇠를 넣지 마세요.** `apiKey`·`authToken` 같은 이름은 거부됩니다. 그 값은
  브라우저로 나가고 백업에 들어갑니다 — 열쇠는 `secret` 타입이나 `.env` 로.
