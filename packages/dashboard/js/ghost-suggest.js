      ...
      (() => {
        ...
        // ★모바일엔 Tab 키가 없다 — 수락 수단이 따로 필요하다. 그런데 **터치를 가로채면
        //  안 된다** (2026-08-10 사용자 실측): 고스트를 탭 가능하게 만들었더니 첫 터치가
        //  textarea 가 아니라 고스트에 맞았고, 그 결과 포커스가 *프로그램적* 으로 옮겨져
        //  iOS 가 입력창을 키보드 위로 올려주지 않았다(가상키보드가 입력창을 덮어, 사용자가
        //  한 번 더 눌러야 했다). 고스트가 없을 땐 멀쩡했다 = 기본 동작은 옳았고 우리가
        //  뺏은 것이다.
        //
        //  그래서 터치는 **그대로 흘려보내고**(고스트는 pointer-events:none 유지) 입력창이
        //  포커스되는 순간에 수락한다. 브라우저 기본 스크롤·키보드 처리가 그대로 돈다 —
        //  뷰포트 계산을 우리가 할 일이 없다.
        const acceptOnTouchFocus = () => {
          try {
