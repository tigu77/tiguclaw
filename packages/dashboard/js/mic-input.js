      ...
      (() => {
        ...
        const insertText = (text) => {
          const t = (text || "").trim();
          if (t === "") { toast(i18n("mic.err.emptyResult"), "warn"); return; }
        ...
        const transcribe = async (blob, mimeType) => {
          ...
          try {
            ...
            if (!r.ok) {
              toast((data && data.error) || i18n("mic.err.request", { status: r.status }), "bad");
              ...
              toast(i18n("mic.err.emptyResponse"), "warn");
            }
          } catch (e) {
            toast(i18n("mic.err.network"), "bad");
          } finally {
        ...
        const onPointerDown = (ev) => {
          ...
          if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            toast(i18n("mic.err.insecure"), "warn");
            return;
          }
          state = "arming";
          armPointerDown = true;
          ...
          navigator.mediaDevices.getUserMedia({ audio: true }).then((s) => {
            ...
            try {
              ...
              try { s.getTracks().forEach((t) => t.stop()); } catch { /* noop */ }
              toast(i18n("mic.err.unsupported"), "bad");
            ...
            if (name === "NotAllowedError" || name === "SecurityError") {
              toast(i18n("mic.err.denied"), "warn");
            } else if (name === "NotFoundError" || name === "NotReadableError") {
              toast(i18n("mic.err.notFound"), "warn");
            } else {
              toast(i18n("mic.err.open"), "bad");
            }
            state = "idle";
            resetUi();
          });
