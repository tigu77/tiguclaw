      ...
      const sendChatMessage = async (text, attachments, replyToText) => {
        ...
        try {
          ...
          // ★★**되돌리진 않되, 침묵하지도 않는다** (2026-09-22 릴리스 레드팀 P-1).
          //  종전엔 여기가 **완전히 조용했다** — 쓴 글은 전송 시점에 비워진 그대로 사라지고,
          //  오류도 안 뜨고, 「작업 중」은 새로고침 전까지 켜져 있었다. 조용한 소실이다.
          //  ★되돌리지 않는 것은 **일부러**다(바로 위 주석 · `sendRejectionAction` 옆의
          //   그물이 «실패로 안 바꾼다» 를 못 박는다): 서버가 받았는지 모르는데 되돌리면
          //   사용자가 다시 눌러 **같은 지시가 두 번** 간다. 그 판단은 그대로 둔다.
          //  ★바꾸는 것은 **말해주는 것 하나**다. 이건 «잘 처리된 일» 이 아니라 «모른다» 이고,
          //   모르는 것은 알려야 한다(2026-09-22 정태님: *"에러는 확실히 나오면 괜찮은데
          //   잘처리된건 소음이지"* — 이건 그 반대편이다).
          renderLocalChat("info", i18n("chat.send.deliveryUnknown"));
        }
        return { ok: true };
      };
