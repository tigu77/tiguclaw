       ...
       *
       * ★**세 판단은 다르다:**
       *  - «되돌리나» — 서버가 **안 받았다고 말한 것**만 되돌린다(아래).
       ...
       *
       * ★★**2026-09-16 2차 — 첫 판은 고치는 쪽이 틀렸다.** 같은 신고를 읽고 «말을 안
       *  해준다» 를 문제로 잡아 `tellUser` 를 켰는데, 실제 문제는 **되돌린다는 것 자체**
       *  였다. 사슬을 양끝으로 읽고서야 보였다:
       *
       *    턴 진행중 전송 → `enqueueThreadTurn` 직렬 큐 → POST 가 그 promise 를 await
       *    → 60초(`HANDLER_TIMEOUT_MS`) → `Promise.race` 가 **504**
       *    → 그런데 `channelHandler` 는 **계속 돈다** = 그 메시지는 실행된다
       *
       *  즉 **504 는 «안 받았다» 가 아니라 «아직 안 끝났다»** 다. 되돌리면 사용자가 다시
       *  보내 **중복 전송**이 된다. 실측: 대시보드 턴 최대 35분 — 긴 턴 중 전송은 거의
       *  항상 이 길로 온다.
       *
       * ★**상태 코드 목록으로 고치지 않는다**(`504` 하나를 더하는 형태). 그건 다음 코드가
       *  생기면 또 뚫리는 «목록 수정»이다. 판정의 **종류**를 바꾼다:
       *
       *    **4xx = 서버가 «안 받았다» 고 말한 것**(400 첨부 캡·빈 본문, 413 크기) → 되돌린다
       *    **5xx = 받았는지 모르는 것**(500 핸들러 throw, 504 시한)          → 안 건드린다
       *
       *  이건 바로 아래 `sendChatMessage` 의 연결-끊김 분기가 이미 지키는 규칙과 **같은
       *  문장**이다 — *"모를 땐 지우지도, 되돌리지도 않는다."* 504 가 그 칸인데 다른
       *  길로 갔던 것이다.
       */
      const sendRejectionAction = (elapsedMs, status, body) => {
        ...
        // ★**등급으로 추측하지 않는다 — 받은 쪽이 말한 것만 믿는다** (2026-09-17 2차 정정).
        //  첫 판은 «4xx=안 받음 / 5xx=모름» 이었는데, 5xx 를 내는 자리가 셋이고 그중 **둘은
        //  확실히 안 받은 것**이었다: 대시보드 프록시의 `502 bridge unreachable`(데몬 정지·
        //  재시작 창)과 브리지의 `503 channel not started`(부팅·종료 창). 컴포저는 전송
        //  직전에 비워지므로 그 둘에서 **쓴 글과 첨부가 영구 소실**됐고, 화면은 «순서대로
        //  실행됩니다» 라는 거짓 안내까지 띄웠다. 배포가 커밋마다라 그 창은 자주 열린다.
        // ★고침은 목록을 넓히는 게 아니라 **판정의 종류를 바꾸는 것**이다. 핸들러가 실제로
        //  돈 응답만 `accepted:true` 를 싣는다(`routes-chat.ts`). 여기선 **그 사실만** 읽는다.
        //  새 5xx 출처가 생겨도 필드를 안 달면 «안 받았다» 로 읽어 글을 지킨다 — 틀리는
        //  방향이 안전한 쪽이다(중복 전송은 보이고 되돌릴 수 있지만, 소실은 둘 다 아니다).
        const b = body && typeof body === "object" ? body : {};
        const accepted = b.accepted === true;
        return {
          restore: !accepted, // 받았다고 **말한** 것만 안 되돌린다.
          tellUser: true, // 명시적 거절은 시간과 무관하게 말한다.
          // ★«아직 돈다» 도 서버가 말한 것만. 즉시 실패한 5xx 가 «처리 중» 으로 보이던 것을
          //  같이 닫는다(적대 검토 P-1 의 나머지 절반 · G-3).
          stillRunning: b.running === true,
          clearWorking: Number(elapsedMs) < IMMEDIATE_MS, // ★이 축은 안 건드린다.
          status: s,
        };
      };

      /**
       * **서버에 도착했는지 확인한 뒤 화면이 할 일** — 순수 (2026-09-22 정태님:
       * *"에러는 확실히 나오면 괜찮은데 잘 처리된 건 소음이지"*).
       *
       * ★도착했으면 **평범한 전송과 결과가 같다.** 글은 서버가 들고 있고, 턴은 돌고 있고,
       *  답은 SSE 로 온다 — 그러면 화면도 평범해야 한다. 안내를 띄우는 건 잘 된 일에 말을
       *  거는 것이고, 「작업 중」을 끄는 건 도는 중인데 **멈춘 것처럼** 보이게 하는 것이다.
       *  실사례: 창을 닫으려다 만 것뿐인데 *"연결이 끊겼지만…"* 이 떴다(실측 09:11:16,
       *  그 턴은 7초 뒤 정상 종료했고 `chat_log` 에 중복도 없었다).
       * ★**판정을 한 곳에 모은 이유**: 종전엔 두 호출부가 **같은 사실에 다르게 굴었다** —
       *  연결 끊김 가지는 「작업 중」을 유지했고, 5xx 가지는 10초 안이면 **껐다.** 같은
       *  «도착했다» 에 판정이 둘이면 한쪽만 고쳐진다
       *  ([[feedback_simple_composable_no_duplication]]).
       * ★**조용한 건 «도착했다» 일 때뿐이다.** 확인이 «아니다» 이거나 확인 자체가 실패하면
       *  종전대로 되돌리고 말한다 — 틀리더라도 «보이고 되돌릴 수 있는» 쪽으로 틀린다.
       */
      // ★블록 본문으로 쓴다 — 회귀가 이 함수만 떼어 실행하는데(`grabFn`), 그 도구가
      //  `const <이름> = (` 부터 `\n      };` 까지로 자른다. 한 줄 화살표로 쓰면 경계가
      //  **옆 함수까지** 넘어가 «이미 선언됨» 으로 죽는다(실제로 한 번 그랬다).
      const arrivalOutcome = (arrived) => {
        const normal = arrived === true; // 문자열 "true"·1 같은 것은 도착이 아니다.
        return { normal, restore: !normal, tellUser: !normal, clearWorking: !normal };
      };
