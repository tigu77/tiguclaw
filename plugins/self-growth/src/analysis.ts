...
export const normalizeErrorMessage = (raw: string): string => {
  return (
    raw
      ...
      // ★UUID 를 **통째로** 먼저 지운다 (2026-08-31). 조각으로 두면 아래 숫자 규칙이
      //  하이픈 사이를 제각각 `<n>` 으로 바꿔 **같은 실패가 매번 다른 키**가 된다
      //  (실측: `268be2e5-…`→`<hash>-<n>-<n>-a<n>cc-<hash>` vs `2a150a64-…`→`<hash>-f<n>df-…`).
      .replace(/\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b/gi, "<uuid>")
      // ★**단어경계에 기대지 않는다** (2026-08-31). 종전 `\b[0-9a-f]{8,}\b` 는 앞에 글자가
      //  붙은 id(`wa2aed889093f` — 매니저 id 형태)를 **못 지웠다**: `w` 와 `a` 사이엔 경계가
      //  없다. 그러면 숫자 규칙이 조각내 `wa<n>aed<n>f` 처럼 **id 마다 다른 결과**를 내고,
      //  멱등 키가 갈려 **같은 실패가 매번 새 메모리**가 된다(실측: 94% 유사한 쌍).
      // ★이 파일이 바로 아래에서 **같은 진단**을 이미 적어뒀다 — *"단어경계 의존 시
      //  `30000ms` 가 마스킹 실패 → 다른 군집키로 분산"*. **숫자엔 고쳤고 해시엔 안 고쳤다.**
      //  16진수 8자 이상 런은 경계 무관하게 지운다(영단어 오탐은 아래 주석 참조).
      // ★**`[a-f]` 를 최소 하나 요구한다** (2026-08-31, 적대 검토 P3). 종전 `[0-9a-f]{8,}`
      //  는 **순수 숫자 8자리 이상**도 먹었고, 그게 아래 «숫자+단위» 규칙보다 먼저 돌아
      //  그 축을 **되깨뜨렸다**: `45000ms`→`<n>` 인데 `100000000ms`→`<hash>ms` 라 같은
      //  타임아웃이 다른 키로 갈렸다. **고치려던 바로 그 결함을 다른 자릿수에서 재발시킨
      //  것**이다. 16진수 id 는 정의상 `a~f` 를 포함하므로 이 조건으로 잃는 게 없다.
      .replace(/(?=[0-9a-f]{8,})[0-9a-f]*[a-f][0-9a-f]*/gi, "<hash>")
      ...
      .replace(/\d+(\.\d+)?/g, "<n>")
      .replace(/\/[^\s]+/g, "<path>")
      // ★**같은 자리의 id 는 한 자리표시자로** (2026-09-01, 3라운드 F11). 위 두 규칙이
      //  `1234567a`(hex 로 보임) → `<hash>` · `12345678`(순수 숫자) → `<n>` 으로 갈라서,
      //  **같은 필드의 id 인데 내용에 따라 키가 달라졌다**(8자 hex id 중 순수 숫자일 확률
      //  `(10/16)^8` = 2.33%). 군집에서 «해시냐 숫자냐» 는 정보가 아니다 — 지우려는 대상
      //  그 자체다. 합치면 그 갈림이 사라진다.
      .replace(/<hash>/g, "<n>")
