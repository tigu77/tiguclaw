...
| **high** | "high/꼼꼼히/제대로", 넓은 diff·브랜치·PR(다파일) | **팬아웃** | 차원별 1명(§B2) | 2명(§B3) |
| **ultra** | "ultra/딥/멀티에이전트/샅샅이" | **와이드 팬아웃** | 차원 × 영역 분할(4~8명) | 3~5명, 2라운드 옵션 |
...
> ★depth 1 제약: 서브에이전트는 다시 spawn 못 한다. **오케스트레이션(차원·회의론자 팬아웃, 집계)은 메인 턴이 직접 수행**한다 — 서브에게 "네가 회의론자를 또 띄워라" 지시 금지.
