...
## 3) 복사 (확인 후)
- **스킬**: `~/.codex/skills/<name>/` 디렉터리 전체 → `<대상>/skills/<name>/`. `SKILL.md` + 보조
  파일(스크립트·레퍼런스) 함께(`Glob` 하위 열거 후 각각 Read/Write). `metadata.short-description`
  등 부가 frontmatter 는 tiguclaw 가 무시 → verbatim 안전.
- **(선택) 프롬프트→커맨드**: `~/.codex/prompts/<name>.md` → `<대상>/commands/<name>.md`.
