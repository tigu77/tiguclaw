...
| **여럿을 동시에 돌리기** | `spawn_agent` 는 **항상 즉시 jobId** 를 준다(기다리지 않는다). 독립인 것은 **전부 띄운 뒤** `wait_for_worker([jobId, …])` 로 **한 번에** 합류 — 하나 띄우고 바로 합류하면 줄을 선다 |
...
**동시에 돌릴 때**도 위 "면적" 기준은 그대로다 — 동시에 돌 수 있다는 게 많이
...
이름은 `settings.json` 의 **프로파일**이고 각자 풀·폴백 체인을 든다(가용 목록은 작동 컨텍스트의 `## 모델 프로파일`). `model: "opus"` 처럼 모델명을 박으면 **에러 없이 디폴트로** 떨어져 등급 의도만 사라진다.
...
그때 맥락을 입혀 자기 인격으로 정리해 보고한다(SYSTEM.md §1). 진행 중 사용자가 다른 걸
물으면 정상 응대한다.
