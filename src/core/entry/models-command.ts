...
const formatPool = (
  ...
  return parts
    .map((e) => {
      ...
      //  화면에 안 보이면, 사용자는 왜 한도가 빨리 닳는지 알 방법이 없다.
      // ★**배수를 적는다** (2026-09-10 정정). 처음엔 «한도 더 씀» 이라고 뭉뚱그렸는데,
      //  공식 문서(learn.chatgpt.com/docs/agent-configuration/speed)는 **크레딧 2.5배**
      //  라고 못박는다(GPT-5.6·5.5·6-Astra 기준. 5.4 는 2배). 「조금 더」와 「2.5배」는
      ...
      //  잘못된 spec 을 이 화면이 따로 짚는 것은 별건이다.
