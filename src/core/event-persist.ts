...
// 값은 리치 카드 최대(diff 60줄/6000자 → JSON+escape ~8KB)를 여유 있게 보존하도록 잡았었다.
//
// ★2026-08-08 상향(10,000 → 64,000) — 사용자 신고: 긴 답변이 대시보드에서 잘려 보였다.
//  12,650자 답변의 **텍스트 세그먼트**(`llm.activity kind:"text"`)가 상한을 넘어, 아래
//  절단이 그 본문을 **300자**로 잘랐다. 300 은 *조회 키*(threadKey·label)를 살리려고 정한
//  값인데 **본문에도 같은 값**이 적용된 것이다. chat_log 엔 전문이 무손실로 있는데 화면은
//  잘린 사본을 그렸고, 잘렸다는 표시도 없어 **정상 완료처럼** 보였다.
//
//  ★왜 이제야 터졌나: 답변 2,025건 중 10,000자를 넘긴 게 **그 한 건**이다(2위 9,788자).
//   상한 바로 아래에서 오래 살았을 뿐, 새 결함이 아니다.
//  ★왜 끄지 않고 올리나: 이벤트는 **개수로만 바운드**(최근 10,000건)라 상한이 없으면 한 건이
//   수십 MB 로 들어올 수 있다. 실측으로는 상한이 걸린 게 7,146건 중 **2건(0.03%)** 이고
//   평균 638자·최대 8,713자라, 64,000 으로 올려도 크기 영향은 사실상 0이면서 폭주는 계속 막는다.
//   (도구 출력은 원천에서 이미 캡: diff ≤7.5KB · output ≤4.5KB.)
export const MAX_PAYLOAD_CHARS = 64000;
