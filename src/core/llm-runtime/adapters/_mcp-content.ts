 ...
 *
 * ★codex 는 이미지를 그 `function_call_output` **바로 뒤**에 두므로 어느 도구 결과인지가
 *  자리로 드러나고, 밀려난 자리엔 `supersededMediaText` 가 남는다. agents-SDK 는 SDK 가
...
export const toolMediaNote = (
  ...
  const gone =
    dropped > 0
      ...
      : "";
  return `(도구 결과 이미지 ${count}장${who}.${gone})`;
};
