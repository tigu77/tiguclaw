    ...
    tools: [
      ...
      tool(
        ...
        "대시보드 **화면 테마**를 고릅니다 — 홈 settings.json 의 `theme`(값은 `themes/<이름>.css` 의 파일명). " +
          ...
          // ★**새 테마를 만들어 달라**는 요청에 갈 곳이 없었다 (2026-08-27 사용자 지적).
          //  종전 설명은 `themes/<이름>.css` 라고만 해서 그게 홈인지 레포인지 알 수 없었고,
          //  `<home>` 이 어딘지도 여기선 안 나온다. 고르기·덮어쓰기는 아는데 **만들기**만
          //  도달 경로가 0이었다.
          "★**새 테마를 만들려면** `<home>/themes/<이름>.css` 에 파일을 쓰면 됩니다 — " +
          "파일을 놓는 순간 목록에 뜹니다(따로 등록하는 절차 없음). 이름은 **영숫자로 시작**하고 " +
          "`[A-Za-z0-9_-]` 만 쓰며 64자 이내입니다 — 공백·`.`·앞 하이픈이 있으면 **목록에서 조용히 " +
          "빠지니** 규칙에 맞춰 지으세요. " +
          "홈 경로는 매 턴 `<env>` 블록의 `tiguclaw home` 에 있습니다. 번들 프리셋(dark·light)은 업데이트가 덮으므로 " +
          "**고치지 말고** 홈에 새로 만드세요.",
        {
          name: z
            .string()
