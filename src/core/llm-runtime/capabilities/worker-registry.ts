...
export const runWorkerJob = (
  ...
  void (async () => {
    ...
    try {
      ...
      //  문제이고, 로그의 «남은 자식» 수가 라운드마다 늘어나는 것으로 보인다.
      ...
      //  개입(steering)은 꺼도 되지만 **결과 거두기는 끌 수 있는 기능이 아니다.**
      {
        ...
        while (
          ...
          //  ★남은 자식 수를 같이 찍는다 — 거두기 중에 이 수가 **늘면** 매니저가 새 위임을
          //   한 것이다([[feedback_logs_must_stand_alone]]: 판정 수치를 실어라). 라이브
          //   사고에서 이 수가 늘었고, 로그로 그걸 볼 방법이 없었다.
          console.log(
          ...
          out = await rerun(
            {
              ...
              //   그 판단이 또 틀리면 로그의 «남은 자식» 수가 늘어나는 것으로 보인다.
              //
