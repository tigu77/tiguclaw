...
export const createWorkerMcpServer = (
  ...
  const runInBackground = tool(
    "run_in_background",
    "**일을 통째로 맡기는 곳입니다** — 매니저는 당신(메인 비서)과 **동급의 전권**으로 작업을 끝까지 책임집니다. ★**`spawn_agent` 과 고르는 기준은 «자식을 또 나눠 붙여야 하나» 입니다.** 할 일이 정해져 있고 그걸 할 전문가가 이미 있으면 `spawn_agent`(그 에이전트는 재위임을 못 합니다). 반대로 **여럿에게 나눠 붙여야 하는 일**이면 매니저입니다 — **한 작업에 서브에이전트가 2명 이상 필요하면 매니저에게 통째로 넘기고 팬아웃은 그 안에서 하세요**(기준은 작동 컨텍스트의 위임 규칙이 정본입니다). 매니저만 `spawn_agent` 으로 자기 자식을 붙일 수 있습니다(매니저가 또 매니저를 띄우는 것만 불가). 상한도 매니저 쪽이 훨씬 깁니다. 둘 다 즉시 jobId 를 돌려주고 백그라운드에서 돕니다 — 결과가 필요하면 `wait_for_worker` 로 합류하세요. 다만 매니저는 오래 도는 게 정상이라 보통 합류하지 말고 사용자에게 바로 «시작했어요» 라고 답한 뒤 대화를 이어가세요(끝나면 결과가 알림으로 돌아옵니다). 발사 후에도 steer_worker 로 지시를 더 얹을 수 있지만 반영은 매니저의 다음 판단 시점이므로(즉시 아님) 범위·판단기준은 task 에 최대한 담아 보내세요. task 에는 사용자 원문 + 매니저가 단독으로 작업하는 데 필요한 맥락을 충분히 적으세요(매니저는 이 대화 history 를 보지 못합니다). **`path`(폴더 경로)를 주면 매니저가 그 폴더 컨텍스트로 실행됩니다 — 그 폴더 전용 스킬/파일작업(상대경로)이 그 폴더 기준이고, 대시보드 그 프로젝트에 귀속되어 보입니다.** 품질이 결과를 좌우하는 작업(코드리뷰·설계·복잡 추론)은 `tier: 'high'`, 단순·대량 작업은 `tier: 'low'` 로 매니저 모델 등급을 지정하세요(미지정 시 기본 모델).",
    {
      task: z
        .string()
        .min(1)
  ...
  // ★`wait_for_worker` 는 **여기 없다** — `agent-registry`(소환과 같은 서버)로 옮겼다
  //  (2026-09-03, 정태님 지적). 이 서버의 도달 범위는 `workers:"main"` 이라 **매니저가 못
  //  닿는다.** 그런데 매니저는 `agents:"manager"` 로 **자식을 띄울 수 있다** — 즉 여기 두면
  //  매니저는 «띄울 수는 있는데 합류할 수 없는» 상태가 된다(실제로 그 상태였다).
  //  **띄우는 자리와 거두는 자리는 같은 도달 범위여야 한다.**
