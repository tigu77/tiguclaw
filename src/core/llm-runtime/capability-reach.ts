...
export const REACH = {
   ...
   * 인데, 진짜 문제는 **뒤의 둘**이었다. 크기가 아니라 **권한** 문제다 — 1턴짜리
   * 서브에이전트가 사용자의 대화를 이름 바꾸거나 보관 처리할 수 있었다.
   * ★읽기/쓰기로 가르지 않았다 — 키가 하나 늘고, `list_sessions` 만 남겨야 할 이유도
   *  약했다. **묶음을 안 쪼갠 것**이지 «읽기도 안 될 일» 이어서가 아니다.
   *  (2026-09-10 적대 검토 정정: 여기 «대화 목록을 **읽거나** 고칠 이유가 없다» 고
   *  적었던 것은 과했다. 같은 «대화 목록 읽기» 인 `list_conversations`·
   *  `search_conversations` 는 `memory` 소속으로 서브까지 닿고 실측 창
   *  2026-08-19~09-10 에 **26번 쓰였다.** 결정은 옳고 문장이 틀렸다.)
