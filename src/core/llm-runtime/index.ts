...
const warnIfPrefixCacheCold = (
  ...
  console.warn(
    `[prefix-cache] 안정 프리픽스가 ${streak}턴 연속 캐시에 안 걸린다 — ` +
      ...
      // ★**«모델을 바꿔 보라» 를 지웠다** (2026-09-10). 종전 이 자리는 «실측상 모델 단위
      //  문제였다(sol 12% ↔ terra 95%+)» 라고 단언했는데, 그 근거는 **11턴짜리 프로브 한
      //  번**이었다. 46일치 실사용(`llm.turn_done`)은 그 말을 뒷받침하지 않는다:
      //  sol 메인 227턴 가중 42%(적중 79턴) · sol 서브 75턴 45% · terra 서브 119턴 52%.
      //  차이가 있어도 7%p 수준이지 8배가 아니다. **틀린 처방을 헌법처럼 적어 두면
      //  모든 사용자가 그 길로 간다** — 실제로 사용자가 이 문구를 받고 «솔이 필요한
      //  상황인데 테라로 내리긴 힘들지» 라고 되물었다([[feedback_verify_before_asserting]]).
      // ★대신 **로그가 이미 답을 갖고 있다.** `codex-turn-end` 의 `갈림=` 이 그 판정이다:
      //  «없음» 이면 우리 프리픽스는 그대로인데 안 걸린 것(우리가 손댈 데가 없다),
      //  «N칸» 이면 우리가 깬 것이고 같은 줄의 `★도구변화=` 가 **어느 도구인지** 말한다.
      //  실측 2026-09-10: 한 설치본의 메인이 `64개↔63개` 를 턴마다 오가며 캐시를
      //  3,712 바닥에 붙여 뒀다 — 모델을 바꿔도 안 고쳐질 부류였다.
      // ★«없음» 을 **«우리 밖»으로 단정하지 않는다** (2026-09-10 적대 검토 P2, 실증).
      //  지문은 **지시+도구**만 덮는다. `input` 앞머리를 바꾸는 경로가 셋인데 그중
      //  하나(도구 출력 압축)만 센다:
      //   ① `compactOldToolOutputs` — 입력 한가운데를 고쳐 쓴다. **센다**(`★압축=N건`).
      //     ★같은 카운터에 `compactOldToolMedia`(도구 이미지 창)도 더해진다 (2026-09-15).
      //      그것도 **입력 한가운데를 고쳐 쓰므로** 여기 세는 것이 맞다 — 다만 `★압축=N건`
      //      한 숫자가 «텍스트 + 미디어» 합계라는 뜻이다(어느 쪽인지는 안 가른다).
      //   ② `CODEX_TURN_HISTORY_CHAR_CAP` 의 oldest-drop — 창이 한 칸 미끄러지면
      //      `input[0]` 부터 바뀐다. **안 센다.** 프로브 실측: 요약 0회·압축 0건인데도
      //      `input[0]` 이 `턴000`→`턴003` 으로 바뀌었다(지시 45,000자 고정).
      //   ③ 롤링 요약 폴드 — 앞부분이 요약문으로 치환된다. **안 센다.**
      //  ★그래서 «없음» 은 «우리 밖» 이 아니라 **«우리가 재는 자리에선 안 변했다»** 다.
      //   긴 스레드일수록 ②·③ 이 흔하므로, 단정하면 가장 흔한 원인을 백엔드 탓으로
      //   돌린다. 이 문구가 지운 «모델을 바꿔 보라» 와 **같은 부류의 재지 않은 단언**이
      //   될 뻔했다([[feedback_verify_before_asserting]]).
      "같은 줄의 `갈림=` 을 보라 — **«N칸»이면 우리가 깬 것이다**(같은 줄 " +
      "`★도구변화=` 가 어느 도구인지 말한다: 턴마다 도구가 붙었다 떨어지면 " +
      "그 뒤 프리픽스가 통째로 무효가 된다). " +
      "★**«없음»은 «우리 밖»이 아니다** — 이 지문은 **지시+도구**만 덮는다. " +
      "같은 줄에 `★압축=N건` 이 있으면 «없음»이어도 **우리가 깬 것**이고, " +
      "그것이 0이어도 **긴 스레드에선 히스토리 창이 미끄러지거나 앞부분이 요약으로 " +
      "치환되면서 입력 앞머리가 바뀔 수 있다**(그 둘은 아직 안 센다). " +
      "예열 2턴 안이거나 짧게 살고 끝나는 위임일 수도 있다.",
  );
};
...
const remainingForKey = (key: string): number => {
  const now = Date.now();
...
export const selectEligiblePool = (
  ...
  const eligible = sized.filter((spec) => {
    const { key, remainingMs: remaining } = cooldownStatus(spec);
