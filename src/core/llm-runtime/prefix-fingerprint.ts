 ...
 *  ★`input` 을 뺀 것은 «판별력» 때문이다 — 매 call 갱신되니 늘 «갈렸다» 만 나온다.
 *  ★★**«input 은 뒤에 붙기만 한다» 는 내가 재지 않고 쓴 말이고 거짓이다** (2026-09-10 적대
 *   검토 P2). `compactOldToolOutputs`(codex history)가 **앞쪽 `function_call_output` 을
 *   제자리에서 고쳐 쓴다** — 프리픽스 한가운데가 바뀐다. 그래서 `갈림=없음` 이 «우리 밖» 을
 *   뜻하지 않는다. 어댑터가 같은 줄에 `★압축=N건` 을 싣는다; N>0 이면 **우리가 깬 것**이다.
 *   ([[feedback_verify_before_asserting]] — 오늘 이 파일에서 두 번째다.)
