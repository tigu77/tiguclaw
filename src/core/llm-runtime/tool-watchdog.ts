...
const toolSlowWarnDefaultMs = (): number =>
  parsePosIntEnv(
    ...
    // ★3분 → 10분 (2026-08-19 사용자 지정: "도구 180초 상한 경고 문구도 10분 정도로").
    //  3분은 **정상 작업이 흔히 넘는 시간**이다(빌드·설치·큰 검색·외부 MCP 왕복). 그래서
    //  경고가 "이상 신호" 가 아니라 배경 소음이 됐고, 소음이 되면 진짜 막힘도 같이 묻힌다.
    //  이 경고가 잡으려는 것은 **사람이 클릭해야 풀리는 막힘**(OS 권한 다이얼로그·외부 앱
    //  미실행)인데, 그 부류는 10분이 지나도 안 풀리므로 임계를 올려도 놓치지 않는다.
    //  ★끊는 게 아니라 알리는 값이다 — 늦게 알리는 대가는 "조금 늦게 안다" 뿐이고,
    //   일찍 알리는 대가는 "아무도 안 읽는 경고" 다.
    600_000,
  );

/**
 * ★임계는 **하나**다 (2026-08-19 사용자 확정: "서브에이전트가 오래 걸리는 건 당연한 거라
 *  경고를 할 필요가 없지 — 이미 백그라운드 잡에서 뭘 하는지 보이잖아. 개별 도구 시간
 *  경고만 10분으로 있어도 될 것 같아").
 *
 *  종전엔 서브에이전트류에만 완화된 임계(300초)를 따로 뒀다. 그건 **경고가 사용자에게
 *  가던 시절**의 완화책이다 — 이제 서브에이전트 지연은 텔레그램 푸시에서도(shouldNotify-
 *  ToolSlow) 대시보드 렌더에서도 빠졌으므로, 임계를 따로 둘 이유가 사라졌다. 남은 소비처는
 *  로그·이벤트(진단)뿐이고 거긴 한 기준이면 충분하다.
 *
 *  ★두 상수는 실제로 갈렸다: 기본을 3분→10분으로 올리자 고정값 300초였던 이쪽이 기본보다
 *   **짧아져**, "원래 오래 걸리는" 도구가 오히려 더 일찍 경고하는 역전이 났다. 이유가
 *   사라진 손잡이는 조정하는 게 아니라 없애는 게 맞다.
 *
 *  ★하드컷 면제(`hardExempt`)는 그대로다 — 그건 "죽이지 마라" 라서 성격이 다르다.
 */
/**
 * ★상수 두 개가 아니라 **관계**로 둔다 (2026-08-19). 기본을 3분→10분으로 올렸더니 고정값
 *  300초였던 이쪽이 **기본보다 짧아져**, "원래 오래 걸리는" 도구가 오히려 더 일찍 경고하는
 *  역전이 났다. 두 숫자를 따로 두면 한쪽만 조정될 때 반드시 갈린다 — 배수로 묶으면
 *  기본을 어떻게 바꿔도 순서가 유지된다.
 */
export const toolSlowWarnMs = (_tool: string): number => toolSlowWarnDefaultMs();
