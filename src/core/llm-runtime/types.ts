 ...
 * - `rate`    — 토큰 **단가**가 그 배수다(claude Opus 5 fast: $10/$50 vs 표준 $5/$25 = 2배).
 *
 * ★**축이 섞이지 않게 주의한다** (2026-09-11 적대 검토 N9). `rate` 는 **API 과금** 축의
 *  숫자다. 그런데 우리 claude 기본 경로는 **구독 OAuth** 이고, 거기서 5시간/7일 한도가
 *  2배로 닳는지는 **아무도 안 쟀다.** codex 쪽은 «크레딧» 으로 축을 구분해 뒀는데 claude
 *  쪽만 섞여 있었다 — 그래서 화면 문구를 «단가» 로 못박지 않고 «더 비싸다» 는 사실만
 *  말한다. 구독 한도 축을 실측하면 그때 단위를 나눠라.
 *
...
export interface ActivityOutput {
  ...
  text: string;
  /**
   * ★**캡 적용 전** 도구 결과의 전체 문자 수 (2026-09-05).
   *
   * `text` 는 프리뷰(40줄·4,000자 캡)라 «이 도구가 컨텍스트에 얼마나 얹었나» 를 못 말한다.
   * 그런데 실측상 **비용의 58%가 정가 input** 이고 그중 86~91%가 «턴이 진행되며 쌓이는
   * 내용» 이다(콜드 프리픽스는 9~14%뿐). 즉 돈을 쥔 것은 프리픽스 크기가 아니라 **도구가
   * 돌려주는 양**인데, 그걸 재는 자리가 없었다.
   *
   * ★문자 수다(토큰 아님) — 토큰은 어댑터·모델마다 다르고 여기선 알 수 없다. 비율은
   *  사후에 곱한다. 「잰 것」과 「환산한 것」을 섞지 않기 위해 원시값만 남긴다.
   * ★`buildActivityOutput` 이 프리뷰를 만들기 **전에** 잰다 — 캡 뒤에 재면 4,000 에서
   *  전부 평평해져 «어느 도구가 큰가» 가 사라진다.
   */
  fullChars?: number;
  /** 크기 캡으로 잘렸나. */
  truncated?: boolean;
