...
const addMemoryTool = tool(
  ...
  "새 사실 메모리 추가 또는 동일 name 존재 시 UPSERT. 저장 위치가 여기로 정해졌으면 「기억할까요?」 묻지 말고 호출. " +
    "★여기는 필요할 때 회수하는 사실(취미·일정·링크·관측) 자리다. " +
    "행동 규범(항상 존댓말·자동 푸시 금지)은 검색어 유무와 무관하게, 전역 상시 지침이면 " +
    "`<home>/AGENT.md`, 프로젝트 지침이면 해당 `PROJECT.md`에 둔다. " +
    "조건부 절차는 해당 스킬에 두고 적용 지점에서 찾을 포인터를 남긴다. " +
    "사실과 규범이 섞이면 분리하고, 불명확하면 확인한다(근거: 헌법 §4).",
  {
