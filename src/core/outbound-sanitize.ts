...
const TOKEN_PATTERNS: { re: RegExp; replace: string }[] = [
  ...
  //  이것 하나만 열려 있었다.
  //  ★**토큰 문자를 열거하지 않는다** (2026-08-15 2차, 적대 검토 P9). 처음엔 base64url
  //  문자만 받게 닫았는데, 그러면 **가리는 힘이 줄었다**: `Bearer 12345:aBcD…` 는 `:` 에서
  //  멈춰 **비밀 절반이 평문으로 남고**, `%2F` 로 인코딩된 토큰도 20자가 그대로 나갔다.
  //  종전 `\S+` 는 둘 다 완전히 가렸다 — 꼬리 먹기를 고치면서 마스킹을 좁혔고 그 손실이
  //  아무 데도 안 적혀 있었다. 이건 로그·DB·**대시보드 SSE 프레임**을 전부 지난다.
  //  ★"무엇이 토큰인가"(열거 — 늙는다)가 아니라 **"어디가 문자열의 끝인가"**(판정)로 바꾼다.
  //  구분자에서 멈추므로 JSON 은 살고(`"` 앞), `:`·`%` 가 든 토큰은 끝까지 가려진다.
