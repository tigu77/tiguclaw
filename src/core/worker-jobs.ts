...
export const listChildJobs = (
  ...
  //  ★계약은 **«기존 항목의 누락·중복이 0»** 이다. 새 자식은 **대개** 안 나온다(커서보다
  //   새로우니까) — 다만 시각이 커서와 **같고** jobId 가 커서보다 작거나, 시계가 뒤로 가면
  //   뒤쪽 쪽에 한 번 나온다. 그건 손상이 아니다(없던 것이 더 보일 뿐).
  //   ★종전 주석은 *"새로 생긴 자식은 안 나온다"* 라고 **재지 않고 단언**했다(외부 검토가
  //    짚었다). 지키는 것과 지켜지길 바라는 것을 문장에서 가르지 않으면, 다음 사람이
  //    그 문장을 근거로 판단한다([[feedback_verify_before_asserting]]).
