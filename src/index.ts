...
const handler: MessageHandler = async (msg) => {
  ...
  if (trimmed === "/models" || trimmed.startsWith("/models ")) {
    ...
    await replyCommand(
      ...
      renderModelProfiles(
        ...
        modelCapsFor,
        // «빠름» 을 **실제로 싣는 자리**만 대가를 적는다 — 판정은 런타임과 같은 함수다
        // (사용자 정의 provider 도 claude 어댑터를 타고, ollama·google 은 openai 어댑터로
        //  오지만 그 손잡이가 없다). 클로저로 여기서 다시 조립하면 운반과 표시가 갈린다.
        speedCostKeyFor,
      ),
    );
    return;
  ...
  if (trimmed.startsWith("/")) {
    ...
    // 명령 **본문**은 `core/entry/slash-commands.ts` 에 있다(구조 감사 ③). 여기 남는 건
    // 조건과 폴스루뿐이고, 문맥은 한 벌로 만들어 넘긴다.
    const slashCtx = { msg, args, trimmed, sidChannel };

    if (cmd === "/memo") {
      await handleMemo(slashCtx);
      return;
    }

    if (cmd === "/forget") {
      await handleForget(slashCtx);
      return;
    }

    if (cmd === "/memos") {
      await handleMemos(slashCtx);
      return;
    }

    if (cmd === "/schedule") {
      await handleSchedule(slashCtx);
      return;
    }
    ...
    if (cmd === "/cooldown") {
      await handleCooldown(slashCtx);
      return;
    }

    if (cmd === "/plugins") {
      await handlePlugins(slashCtx);
      return;
    }
