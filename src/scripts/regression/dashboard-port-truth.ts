 ...
 * 하나가 아니라 **같은 값이 세 숫자**였다 — 두 달을 그렇게 살았다:
 *   코드(`plugins/`·`packages/`) `3101` / 설치 마법사가 쓰는 `.env` `3000` /
 *   배포되는 `packages/dashboard/README.md` `3002`("현재 dev 값" — 내 기계 설정이 제품 문서에)
 * 그래서 **문서에 한 숫자를 쓸 수가 없었다.** 여는 법이 안 적힌 진짜 이유가 이거다.
 *
 * ★고친 방식 = 숫자를 맞추는 게 아니라 **중복을 없앤다**. 마법사는 이제 기본 포트를 `.env` 에
 *  적지 않고(주석으로만 안내), 코드 기본값 하나가 정본이다. 적어두는 순간 갈라진다.
 *
 * ★이 검사는 이름을 열거하지 않는다 — `DASHBOARD_PORT` 를 **말하는 모든 추적 파일**을 찾아
 *  거기 적힌 숫자가 코드 기본값과 같은지 본다. 새 파일이 생겨도 저절로 걸린다.
 *  (`docs/decisions/` 는 제외 — 그건 당시를 적은 **기록**이라 현재값으로 고치면 안 된다.)
 */
...
export const check: RegressionCheck = {
  ...
  guards:
    "대시보드 포트가 코드 3101 / 설치 마법사 3000 / 배포 문서 3002 로 갈라져 여는 법을 못 적던 것",
