 ...
 *   ③**실제 왕복 1회**(의미 — 타입이 안 바뀌는 변화는 이것으로만 보인다).
 *
 *  ★③의 **수단이 생겼다** (2026-09-11): `npm run e2e:openrouter`.
 *   종전엔 «사람이 돌려라» 라고만 적혀 있었고, 실제로는 돌릴 방법이 없어서 한 번도 안
 *   돌았다 — `e2e:ollama` 는 컨텍스트가 작아 **프롬프트가 잘리고**(도구 정의가 날아간다),
 *   구독 경로는 인증이 플러그인 배선을 타서 **채널 서비스가 같이 뜬다**(텔레그램이 떠서
 *   라이브 폴링을 409 로 죽일 뻔했다). OpenRouter 는 **API 키 기반 OpenAI 호환**이라 그
 *   사슬을 안 타고, 무료·도구지원·대용량 모델로 **잘림 0·비용 0** 으로 잰다.
 *   보는 것 셋은 전부 **실제로 사고가 났던 자리**다: 도구 호출 · 멀티턴 히스토리 재생
 *   (앞 턴의 도구 결과를 이어받나) · `usage.outputTokens`(2026-08-05 에 1로 붕괴).
 *
 *  ★★**이 수단은 `@openai/agents` 축만 덮는다**(2026-09-11 적대 검토 G6). 이 검사는 *모든*
 *   의존성을 보는데, `e2e:openrouter` 가 도는 경로는 `{adapter:"openai"}` 하나다.
 *   `@anthropic-ai/claude-agent-sdk` 가 올라갈 때 이걸 돌리고 «③ 했다» 로 넘어가면 **엉뚱한
 *   것을 잰 것**이다 — 위에 근거로 든 usage 붕괴 사고가 바로 그 claude SDK 것이라 더 그렇다.
 *   ★claude·codex 축의 ③ 은 아직 수단이 없다: 구독 인증이 플러그인 배선을 타고, 배선하면
 *    채널 서비스가 뜬다. 그 둘을 잴 때는 **어느 어댑터를 실제로 지나는지** 먼저 확인해라
 *    (codex 는 SDK 위임을 폐기하고 raw fetch 로 돈다 — `@openai/agents` 를 안 로드한다).
 *
