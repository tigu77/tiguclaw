...
export const check: RegressionCheck = {
  ...
  run: async (): Promise<Assertion[]> => {
    ...
    const walk = async (dir: string): Promise<void> => {
      for (const e of await readdir(dir, { withFileTypes: true })) {
        ...
        if (e.isDirectory()) {
          ...
          if (!/^(node_modules|dist)$/.test(e.name)) await walk(p);
          continue;
        }
    ...
    const EXPECT_READ_ONLY = new Set([
      ...
      // 세션 목록 조회 — 순수 읽기.
      "list_sessions",
      // 화면 관측 — 보기만 한다(클릭·입력 없음). 프레임 파일을 쓰지만 그건 우리 기록이고
      // 되불러도 무해하다. 조작은 **별도 이름**(`do`)이라 여기 자동으로 안 딸려온다.
      "look",
    ]);
    const EXPECT_SIDE_EFFECT = new Set([
      // ★`do` — 클릭·글자·키를 **실제로 낸다.** 되돌릴 수 없으므로 폴백이 되부르면 안 된다.
      "do",
      ...
      "set_theme",
      // 홈 위젯 배치 — `set_theme` 과 **똑같은 모양**이다. `widgets` 를 비우면 조회지만
      // 분류는 도구 단위이고, 쓰는 갈래가 settings.json 을 건드리므로 부작용으로 둔다.
      "configure_home",
      // 날씨 조회 — **읽기처럼 보이지만 부작용이다** (2026-08-28, 위젯 플랫폼 증분 2).
      //  외부에서 값을 읽어오기만 하는 게 아니라 **대화에 카드를 발행한다**. 폴백이 되부르면
      //  같은 카드가 두 장 뜨고 남의 API 도 두 번 부른다. 이름(`_lookup`)이 읽기처럼 읽히는
      //  것이 함정이라 여기에 근거를 남긴다 — 분류는 **이름이 아니라 하는 일**로 한다.
      "weather_lookup",
      // 지도 조회 — `weather_lookup` 과 **같은 이유**로 부작용이다(카드를 발행한다).
      ...
      "wait_for_worker",
      // ★★**조작 도구 넷** (2026-09-17). 이 표에서 가장 명백한 부작용이다 — 다른 것들은
      //  «되돌릴 수 있지만 상태를 바꾼다» 라서 보수적으로 분류한 것인데, 이쪽은 **되돌릴
      //  수단 자체가 없다.** 클릭은 취소가 없고, 폴백이 되부르면 **두 번 눌린다.**
      //  ★그리고 재실행이 특히 나쁘다: 앞 시도가 어디까지 갔는지 모르는 상태에서 되부르면
      //   화면이 이미 바뀌어 있어 **다른 것을 누른다**(그래서 플러그인 자신도 «불확실하면
      //   재시도 말고 재관측» 으로 정했다 — 설계 §14-2).
      // ★★도구 다섯(`click`·`type_text`·`key`·`scroll`·`drag`)은 2026-09-19 에 `do` 의
      //  **step 원소로 녹아 없어졌다.** 없는 이름을 표에 두면 «분류돼 있다» 는 착시만 남고,
      //  그 착시가 다음 사람에게 «이 표는 최신이다» 로 읽힌다.
      //  ★드래그가 이 중에서도 특히 나빴다는 것(되부르면 창이 두 번 옮겨진다)은 이제
      //   `do` 가 통째로 진다 — 그래서 `do` 하나로 충분하다.
    ]);
