...
export const check: RegressionCheck = {
  ...
  run: async (): Promise<Assertion[]> => {
    ...
    return [
      ...
      // ★계약이 바뀌었다 (2026-09-03 정태님: *"0이면 메모리 인덱스가 하나도 안 들어가는 거고"*).
      //  종전엔 0을 «오타» 로 보고 기본값으로 되돌렸는데, 그건 **끄겠다는 의사**와 오타를
      //  같은 것으로 본 것이다. 설정 화면에 슬라이더가 생기면서 0은 **도달 가능한 선택**이
      //  됐고, 선택은 막는 게 아니라 그 뜻대로 따른다.
      //  ★음수는 **여전히 오타다** — 사람이 «-1KB 로 하겠다» 고 고를 길이 없다.
      assert(
        "★★`0` 은 **끄겠다는 의사**다 — 그대로 따른다(비서는 검색으로만 기억에 닿는다)",
        withSettings(0) === 0,
        `0→${String(withSettings(0))} (0이어야)`,
      ),
      assert(
        "★★음수는 **여전히 오타** — 고를 수 없는 값이므로 기본값으로 되돌린다",
        withSettings(-1) === DEFAULT,
        `-1→${String(withSettings(-1))}`,
      ),
      assert(
