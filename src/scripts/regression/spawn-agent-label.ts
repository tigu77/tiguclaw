  ...
  run: async (): Promise<Assertion[]> => {
    ...
    return [
      ...
      assert(
        ...
        // ★**이모지 리터럴을 박지 않는다** (2026-09-10). 아이콘이 카탈로그로 올라가면서
        //  `"🤖 " + nm` 이 `ai + nm` 이 됐고, 판정이 그 표현에 묶여 빨개졌다 — 코드가 틀린 게
        //  아니라 검사가 표현을 지키고 있었다(오늘 세 번째다). 지키려는 성질은 «이름과 제목을
        //  같이 보여주는가» 이지 «어떤 글자로 접두를 붙이는가» 가 아니다.
        /opts\.label[\s\S]{0,300}?\bnm\s*\+\s*" · "\s*\+\s*title/.test(dash),
